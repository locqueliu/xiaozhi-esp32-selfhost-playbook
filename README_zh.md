...
这个仓库主要是我在记 `xiaozhi-esp32` 自托管部署这条线上的一些东西。

真正在折腾这类 ESP32 设备时，很多问题不在“能不能跑起来”，而在“后面好不好维护”。比如 OTA 默认指向哪边、WebSocket 该连哪里、运行时配置没给全的时候怎么回退，这些小地方如果不提前想清楚，后面每换一次环境都会重新踩坑。

所以我把这部分单独拎了出来，免得散在分支改动里，过一阵子自己都找不回来。
...
真正做固件还是应该从上游仓库开始，这里主要放我在部署、自托管和路由处理这部分慢慢整理出来的笔记和示例。
